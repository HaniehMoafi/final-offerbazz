...
//        entityManager.remove(entity);
    ...
    @Override
    public boolean duplicateName(ProductEntity p) throws BaseException {
        TypedQuery query =entityManager.createNamedQuery("findProductByName", ProductEntity.class);
        query.setParameter("name1",p.getProductName());
        List<ProductEntity> products = query.getResultList();
       if (Objects.isNull(products)|| products.size()==0)
           return false;
       else if(products.size()==1 && products.get(0).getId().equals(p.getId())){
           return false;
